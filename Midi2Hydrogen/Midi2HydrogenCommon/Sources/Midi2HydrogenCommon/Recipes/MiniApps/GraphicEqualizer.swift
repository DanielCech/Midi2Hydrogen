...
struct GraphicEqualizerView: View {
    ...
    var body: some View {
        VStack {
            ...
            HStack {
                Midi2HydrogenKnob(text: "Band 1",
                                parameter: $conductor.data.gain1,
                                range: 0 ... 20)
                Midi2HydrogenKnob(text: "Band 2",
                                parameter: $conductor.data.gain2,
                             range: 0 ... 20)
                Midi2HydrogenKnob(text: "Band 3",
                                parameter: $conductor.data.gain3,
                             range: 0 ... 20)
                Midi2HydrogenKnob(text: "Band 4",
                                parameter: $conductor.data.gain4,
                             range: 0 ... 20)
                Midi2HydrogenKnob(text: "Band 5",
                                parameter: $conductor.data.gain5,
                             range: 0 ... 20)
                Midi2HydrogenKnob(text: "Band 6",
                                parameter: $conductor.data.gain6,
                             range: 0 ... 20)
            }.padding(5)
            FFTView(conductor.fader)
        }.midi2HydrogenNavBarTitle("Graphic Equalizer")
            .onAppear {
                conductor.start()
            }
            .onDisappear {
