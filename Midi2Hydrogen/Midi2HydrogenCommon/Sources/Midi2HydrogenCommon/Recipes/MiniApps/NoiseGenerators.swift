...
struct NoiseGeneratorsView: View {
    ...
    var body: some View {
        VStack {
            HStack {
                Midi2HydrogenKnob(text: "Brownian", parameter: self.$conductor.data.brownianAmplitude, range: 0...1)
                Midi2HydrogenKnob(text: "Pink", parameter: self.$conductor.data.pinkAmplitude, range: 0...1)
                Midi2HydrogenKnob(text: "White", parameter: self.$conductor.data.whiteAmplitude, range: 0...1)
            }.padding(5)
            NodeOutputView(conductor.mixer)
        }.midi2HydrogenNavBarTitle("Noise Generators")
            .onAppear {
                conductor.start()
            }
            .onDisappear {
