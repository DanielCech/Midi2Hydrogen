...
struct StereoDelayOperationView: View {
    ...
    var body: some View {
        VStack {
            ...
            HStack(spacing: 20) {
                Midi2HydrogenKnob(text: "Left Time",
                                parameter: self.$conductor.data.leftTime,
                                range: 0 ... 0.3,
                                units: "Seconds")
                Midi2HydrogenKnob(text: "Left Feedback",
                                parameter: self.$conductor.data.leftFeedback,
                                range: 0 ... 1,
                                units: "%")
            }
            HStack(spacing: 20) {
                Midi2HydrogenKnob(text: "Right Time",
                                parameter: self.$conductor.data.rightTime,
                                range: 0 ... 0.3,
                                units: "Seconds")
                Midi2HydrogenKnob(text: "Right Feedback",
        ...
        .padding()
        .midi2HydrogenNavBarTitle("Stereo Delay Operation")
        .onAppear {
            conductor.start()
        }
        .onDisappear {
