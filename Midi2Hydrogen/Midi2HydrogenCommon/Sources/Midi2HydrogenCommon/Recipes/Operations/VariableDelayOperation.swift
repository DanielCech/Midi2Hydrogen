...
struct VariableDelayOperationView: View {
    ...
    var body: some View {
        VStack {
            ...
            HStack(spacing: 20) {
                Midi2HydrogenKnob(text: "Max Time",
                                parameter: self.$conductor.data.maxTime,
                                range: 0 ... 0.3,
                                units: "Seconds")
                Midi2HydrogenKnob(text: "Frequency",
                                parameter: self.$conductor.data.frequency,
                                range: 0 ... 1,
                                units: "Hz")
                Midi2HydrogenKnob(text: "Feedback Frequency",
                                parameter: self.$conductor.data.feedbackFrequency,
                                range: 0 ... 1,
                                units: "Hz")
                Midi2HydrogenKnob(text: "Mix",
        ...
        .padding()
        .midi2HydrogenNavBarTitle("Variable Delay Fun")
        .onAppear {
            conductor.start()
        }
        .onDisappear {
